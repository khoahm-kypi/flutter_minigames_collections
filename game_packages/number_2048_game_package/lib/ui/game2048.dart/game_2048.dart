...
class _MainGameWidgetState extends State<MainGameWidget> {
  ...
  Size sizeOfBoard() {
    ...
    return Size(width as double, width);
  }
}
...
class AnimatedCell extends AnimatedWidget {
  ...
  AnimatedCell(
      {Key? key, this.cell, this.state, required Animation<double> animation})
  ...
  Widget build(BuildContext context) {
    ...
    double width = (boardSize.width - (state!.column + 1) * state!.PadCell) /
        state!.column;
    if (cell!.number == 0) {
      return Container();
    } else {
